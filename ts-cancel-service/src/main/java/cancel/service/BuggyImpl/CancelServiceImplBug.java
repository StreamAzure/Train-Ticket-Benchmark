 ...
 * @author fdse
 */
public class CancelServiceImplBug implements CancelService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(CancelServiceImplBug.class);
    ...
    public Response cancelOrder(String orderId, String loginId, HttpHeaders headers) {
        ...
        if (orderResult.getStatus() == 1) {
            CancelServiceImplBug.LOGGER.info("[cancelOrder][Cancel Order, Order found G|H]");
            ...
            if (order.getStatus() == OrderStatus.NOTPAID.getCode()
                ...
                if (changeOrderResult.getStatus() == 1) {
                    ...
                    CancelServiceImplBug.LOGGER.info("[cancelOrder][Cancel Order Success]");
                    //Draw back money
                    String money = calculateRefund(order);
                    boolean status = drawbackMoney(money, loginId, headers);
                    if (status) {
                        CancelServiceImplBug.LOGGER.info("[cancelOrder][Draw Back Money Success]");
                        ...
                        CancelServiceImplBug.LOGGER.error("[cancelOrder][Draw Back Money Failed][loginId: {}, orderId: {}]", loginId, orderId);
                    }
                    return new Response<>(1, "Success.", "test not null");
                } else {
                    CancelServiceImplBug.LOGGER.error("[cancelOrder][Cancel Order Failed][orderId: {}, Reason: {}]", orderId, changeOrderResult.getMsg());
                    return new Response<>(0, changeOrderResult.getMsg(), null);
                }

            } else {
                CancelServiceImplBug.LOGGER.info("[cancelOrder][Cancel Order, Order Status Not Permitted][loginId: {}, orderId: {}]", loginId, orderId);
            ...
            if (orderOtherResult.getStatus() == 1) {
                CancelServiceImplBug.LOGGER.info("[cancelOrder][Cancel Order, Order found Z|K|Other]");

                Order order = orderOtherResult.getData();
                if (order.getStatus() == OrderStatus.NOTPAID.getCode()
                        || order.getStatus() == OrderStatus.PAID.getCode() || order.getStatus() == OrderStatus.CHANGE.getCode()) {

                    CancelServiceImplBug.LOGGER.info("[cancelOrder][Cancel Order, Order status ok]");
...
//                    Response changeOrderResult = cancelFromOtherOrder(order, headers);
                    ...
                    if (status) {
                        CancelServiceImplBug.LOGGER.info("[Draw Back Money] Success.");
                        return new Response<>(1, "Success.", null);
                    } else {
                        CancelServiceImplBug.LOGGER.error("[Draw Back Money] Fail, loginId: {}, orderId: {}", loginId, orderId);
...
//                }
                    ...
                    CancelServiceImplBug.LOGGER.warn("[cancelOrder][Cancel Order, Invalid order status code][loginId: {}, orderId: {}, orderStatusCode: {}]", loginId, orderId, order.getStatus());
                    return new Response<>(0, "Order Not Found.", null);
                }
            }else {
                CancelServiceImplBug.LOGGER.warn("[cancelOrder][Error! Cancel Order, Order Not Found][loginId: {}, orderId: {}]", loginId, orderId);
                return new Response<>(0, "Order Not Found.", null);
            }
        }
    }
    ...
    public boolean sendEmail(NotifyInfo notifyInfo, HttpHeaders headers) {
        CancelServiceImplBug.LOGGER.info("[sendEmail][Send Email]");
    ...
    public Response calculateRefund(String orderId, HttpHeaders headers) {
        ...
        if (orderResult.getStatus() == 1) {
            ...
            if (order.getStatus() == OrderStatus.NOTPAID.getCode()
                ...
                if (order.getStatus() == OrderStatus.NOTPAID.getCode()) {
                    CancelServiceImplBug.LOGGER.info("[calculateRefund][Cancel Order, Refund Price From Order Service.Not Paid][orderId: {}]", orderId);
                    return new Response<>(1, "Success. Refoud 0", "0");
                } else {
                    CancelServiceImplBug.LOGGER.info("[calculateRefund][Cancel Order, Refund Price From Order Service.Paid][orderId: {}]", orderId);
                    return new Response<>(1, "Success. ", calculateRefund(order));
                }
            } else {
                CancelServiceImplBug.LOGGER.info("[calculateRefund][Cancel Order Refund Price Order.Cancel Not Permitted][orderId: {}]", orderId);
            ...
            if (orderOtherResult.getStatus() == 1) {
                ...
                if (order.getStatus() == OrderStatus.NOTPAID.getCode()
                        || order.getStatus() == OrderStatus.PAID.getCode()) {
                    if (order.getStatus() == OrderStatus.NOTPAID.getCode()) {
                        CancelServiceImplBug.LOGGER.info("[calculateRefund][Cancel Order, Refund Price From Order Other Service.Not Paid][orderId: {}]", orderId);
                        return new Response<>(1, "Success, Refound 0", "0");
                    } else {
                        CancelServiceImplBug.LOGGER.info("[Cancel Order][Refund Price From Order Other Service.Paid][orderId: {}]", orderId);
                        return new Response<>(1, "Success", calculateRefund(order));
                    }
                } else {
                    CancelServiceImplBug.LOGGER.warn("[Cancel Order][Refund Price, Order Other. Cancel Not Permitted][orderId: {}]", orderId);
                    return new Response<>(0, orderStatusCancelNotPermitted, null);
                }
            } else {
                CancelServiceImplBug.LOGGER.error("[Cancel Order][Refund Price][Order not found][orderId: {}]", orderId);
                return new Response<>(0, "Order Not Found", null);
            }
        }
    }
    ...
    private String calculateRefund(Order order) {
        ...
        CancelServiceImplBug.LOGGER.info("[calculateRefund][Cancel Order][Order Travel Date: {}]", order.getTravelDate().toString());
        ...
        Date startTime = new Date(year,  //NOSONAR
                ...
                minute,
                second);
        CancelServiceImplBug.LOGGER.info("[calculateRefund][Cancel Order][nowDate  : {}]", nowDate);
        CancelServiceImplBug.LOGGER.info("[calculateRefund][Cancel Order][startTime: {}]", startTime);
        if (nowDate.after(startTime)) {
            CancelServiceImplBug.LOGGER.warn("[calculateRefund][Cancel Order, Ticket expire refund 0]");
            return "0";
        } else {
            ...
            CancelServiceImplBug.LOGGER.info("[calculateRefund][calculate refund][refund: {}]", str);
            return str;
        }
    }
    ...
    private Response cancelFromOrder(Order order, HttpHeaders headers) {
        CancelServiceImplBug.LOGGER.info("[cancelFromOrder][Change Order Status]");
        order.setStatus(OrderStatus.CANCEL.getCode());
    ...
    private Response cancelFromOtherOrder(Order order, HttpHeaders headers) {
        CancelServiceImplBug.LOGGER.info("[cancelFromOtherOrder][Change Order Status]");
        order.setStatus(OrderStatus.CANCEL.getCode());
        HttpHeaders newHeaders = getAuthorizationHeadersFrom(headers);
        HttpEntity requestEntity = new HttpEntity(order, newHeaders);
    ...
    public boolean drawbackMoney(String money, String userId, HttpHeaders headers) {
        CancelServiceImplBug.LOGGER.info("[drawbackMoney][Draw Back Money]");
    ...
    public Response<User> getAccount(String orderId, HttpHeaders headers) {
        CancelServiceImplBug.LOGGER.info("[getAccount][Get By Id][orderId: {}]", orderId);
        HttpHeaders newHeaders = getAuthorizationHeadersFrom(headers);
        HttpEntity requestEntity = new HttpEntity(newHeaders);
    ...
    private Response<Order> getOrderByIdFromOrder(String orderId, HttpHeaders headers) {
        CancelServiceImplBug.LOGGER.info("[getOrderByIdFromOrder][Get Order][orderId: {}]", orderId);
        HttpHeaders newHeaders = getAuthorizationHeadersFrom(headers);
        HttpEntity requestEntity = new HttpEntity(newHeaders);
        String order_service_url = getServiceUrl("ts-order-service");
    ...
    private Response<Order> getOrderByIdFromOrderOther(String orderId, HttpHeaders headers) {
        CancelServiceImplBug.LOGGER.info("[getOrderByIdFromOrderOther][Get Order][orderId: {}]", orderId);
        HttpHeaders newHeaders = getAuthorizationHeadersFrom(headers);
        HttpEntity requestEntity = new HttpEntity(newHeaders);
        String order_other_service_url = getServiceUrl("ts-order-other-service");
        ResponseEntity<Response<Order>> re = restTemplate.exchange(
